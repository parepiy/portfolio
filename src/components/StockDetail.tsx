...
interface Props { stock: Stock; onBack: () => void; }

function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="card p-3">
      <div className="text-xs mb-1 text-faint">{label}</div>
...
function ZoneBanner({ stock }: { stock: Stock }) {
  ...
  let color = "#d97706"; let bg = "#fef3c7"; let border = "#fde68a";
  let desc  = "Price is between buy and sell zones. Monitor for direction.";

  if (inBuy)    { label="Buy Zone — Consider Buying";  color="#047857"; bg="#d1fae5"; border="#6ee7b7"; desc="Price is in the recommended buy zone. Risk/reward favorable."; }
  if (inSell)   { label="Sell Zone — Consider Selling"; color="#be123c"; bg="#ffe4e6"; border="#fda4af"; desc="Price is in the recommended sell zone. Consider taking profit."; }
  if (belowBuy) { label="Below Buy Zone — High Risk";   color="#9a3412"; bg="#ffedd5"; border="#fdba74"; desc="Price has broken below buy zone. Exercise caution."; }
  if (aboveSell){ label="Above Sell Zone — Overbought"; color="#6b21a8"; bg="#f3e8ff"; border="#d8b4fe"; desc="Price extended above sell zone."; }
...
function SentimentMeter({ stock }: { stock: Stock }) {
  ...
  const color = avg > 0.2 ? "var(--up)" : avg < -0.2 ? "var(--down)" : "#d97706";
  ...
  return (
    <div className="card p-4">
      ...
      <div className="flex rounded-full overflow-hidden h-2 mb-2" style={{ background: "var(--border)" }}>
        <div style={{ width: `${(bullN/total)*100}%`, background: "var(--up)" }} />
        <div style={{ width: `${((total-bullN-bearN)/total)*100}%`, background: "#fcd34d" }} />
        <div style={{ width: `${(bearN/total)*100}%`, background: "var(--down)" }} />
      </div>
      <div className="flex justify-between text-xs">
        <span className="text-up">{bullN} Bullish</span>
        <span className="text-down">{bearN} Bearish</span>
      </div>
    </div>
  );
}
...
export default function StockDetail({ stock, onBack }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-4 pt-4 pb-3 shrink-0" style={{ borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}>
        <div className="flex items-start gap-3">
          <button onClick={onBack} className="p-2 rounded-xl mt-0.5 transition-colors" style={{ background: "var(--mint-bg)", color: "var(--mint)" }}>
            <ArrowLeft size={16} />
          </button>
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline gap-2">
              <span className="font-bold text-lg" style={{ color: "var(--text)" }}>{stock.symbol}</span>
              <span className="text-sm text-dim truncate">{stock.name}</span>
